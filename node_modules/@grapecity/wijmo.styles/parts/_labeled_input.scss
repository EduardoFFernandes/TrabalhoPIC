...
.wj-labeled-input {
    ...
    .accesskey { // underline access keys
        text-decoration: underline;
        font-size: unset;
    }

    .wj-control.wj-content { // labeled Wijmo controls
        margin: 0;
        width: 100%;
        background-color: transparent;
        border: none;
        border-bottom: 1px solid rgba(0, 0, 0, .1);
        button { // buttons in Wijmo controls
            opacity: 0.75;
    ...
    .wj-state-focused+label:after { // show underline when focused
        left: 0;
        visibility: visible;
        width: 100%;
    }
    .wj-error { // error messages
        color: $wj-invalid;
        position: absolute;
        font-size: 12px;
        margin-top: 3px;
    ...
    input[type=checkbox], input[type=radio] {    
        ...
        &:checked + label:after { // checked checkbox
            content: '\2714';
    ...
    input[type=radio] {
        ...
        &:checked + label:after { // checked radio
            content: '';
            background: white;
    ...
    &.switch input[type=checkbox] {
        ...
        &:checked + label:before { // checked track
            background: $wj-sel-bkg;
            opacity: .4;
        }
        &:checked + label:after { // checked thumb
            margin-left: 1.25em;
